...
export default function Product({ products }) {
  ...
  return (
    <main className="py-5 dark:bg-slate-950">
      <div className="container mx-auto">
        <div className="flex flex-wrap lg:gap-3 justify-center md:justify-normal">
          <div className="mt-6 lg:mt-0 lg:px-2 lg:w-full">
            <div className="grid grid-cols-1 gap-8 mt-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {products?.map(product => (
                <div key={product._id} className="mx-auto mt-11 relative p-7 w-72 transform overflow-hidden rounded-lg bg-white dark:bg-slate-800 shadow-md duration-300 hover:scale-105 hover:shadow-lg">
                  <Link to={`/ProductDetails/${product._id}`}>
                    <ImgSlider imgs={product.images} answer={false} />
                  </Link>
